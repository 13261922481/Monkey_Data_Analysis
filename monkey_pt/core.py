...
class Table(Canvas):
    ...
    def __init__(self, parent=None, model=None, dataframe=None,
                   ...
                   editable=True, enable_menus=True, notebook=None, dp_main=None,
                   parent_root=None,
                   **kwargs):
    ...
    def show(self, callback=None):
        ...
        self.grid(row=1,column=1,rowspan=1,sticky='news',pady=0)
        ...
        if self.showstatusbar == True:
            self.statusbar = statusBar(self.parentframe, self, dp_main=self.dp_main)
    ...
    def remove(self):
        ...
        if self.dp_main != None:
            self.dp_main.Viewed.set(False)
        return
    ...
    def manipulate_data(self):
        md_root = Toplevel(self.parentframe)
        w = 300
        h = 300
        #setting main window's parameters
        ws = md_root.winfo_screenwidth() # width of the screen
        hs = md_root.winfo_screenheight() # height of the screen
        x = (ws/2) - (w/2)
        y = (hs/2) - (h/2) - 30
        md_root.geometry('%dx%d+%d+%d' % (w, h, x, y))
        md_root.lift()
        md_root.focus_force()
        md_root.resizable(False, False)
        md_root.title('Data Manipulation')

        md_frame = Frame(md_root, width=w, height=h)
        md_frame.place(x=0, y=0)

        df_name = self.dp_main

        Label(md_frame, text='Dataframe variable: d_f').place(x=60, y=30)
        Label(md_frame, text='Python code:').place(x=10, y=75)

        code_entry = scrolledtext.ScrolledText(md_frame, width=38, height=7)
        code_entry.place(x=10, y=100)

        def run_update():
            code_line = code_entry.get("1.0",'end').replace('d_f', 'self.model.df')
            exec(code_line)
        ...
        def test_print():
            print(code_entry.get("1.0",'end'))

        run_button = Button(md_frame, text='Run', width=5, command=lambda: run_update())
        run_button.place(x=80, y=250)
        print_button = Button(md_frame, text='Close', width=5, command=lambda: md_root.destroy())
        print_button.place(x=180, y=250)
    ...
    def aggregate(self):
        ...
        dlg = AggregateDialog(self, df=self.model.df, notebook=self.notebook)
        return
    ...
    def createChildTable(self, df, title=None, index=False, out=False, notebook=None):
        """Add the child table"""
    ...
    def tableFromSelection(self):
        ...
        if len(df) <=1:
            df = pd.DataFrame()
        self.createChildTable(df, 'selection', notebook=self.notebook)
        return
...
class ToolBar(Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        ...
        img = images.fit()
        addButton(self, 'Manipulate Data', self.parentapp.manipulate_data, img, 'manipulate data\nwith python')
        #img = images.prefs()
...
class statusBar(Frame):
    """Status bar class"""
    def __init__(self, parent=None, parentapp=None, dp_main=None):
        ...
        fr = Frame(self)
        fr.pack(fill=Y, side=RIGHT, padx=50)
        #fr.pack_propagate(0)
