...
class rgr_app:
    ...
    class Grid_search:
        def __init__(self, prev, parent):
            ...
            ttk.Label(self.frame, text='Method', font=myfont1).place(x=100, y=10)
            ...
            self.combobox1.place(x=180,y=12)
    ...
    def __init__(self, parent):
        ...
        self.cbr_reg_lambda = tk.StringVar(value='3.0')
            ...
            try:
                    ...
                    if self.x_st_var.get() == 'Yes':
                            ...
                            for fold in folds_list:
                                if self.continue_flag:
                                    lr_scores = np.append(lr_scores, 
                                        cross_val_score(lr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                    self.pb1.step(1)
                                ...
                                if self.continue_flag:
                                    lr_scores = np.append(lr_scores, 
                                        cross_val_score(lr, prev.X_St, 
                                            np.log(prev.y), scoring='r2', cv=fold))
                                    self.pb1.step(1)
                        ...
                        if self.lr_function_type.get() == 'Linear':
                            for fold in folds_list:
                                if self.continue_flag:
                                    lr_scores = np.append(lr_scores, 
                                        cross_val_score(lr, prev.X, prev.y, scoring='r2', cv=fold))
                                    self.pb1.step(1)
                        elif self.lr_function_type.get() == 'Polynomial':
                            from sklearn.preprocessing import PolynomialFeatures
                            for fold in folds_list:
                ...
                if self.rr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                rr_scores = np.append(rr_scores, 
                                    cross_val_score(rr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                rr_scores = np.append(rr_scores, 
                                    cross_val_score(rr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.lassor_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                lassor_scores = np.append(lassor_scores, 
                                    cross_val_score(lassor, prev.X_St, prev.y, 
                                        scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                lassor_scores = np.append(lassor_scores, 
                                    cross_val_score(lassor, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.rfr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                rfr_scores = np.append(rfr_scores, 
                                    cross_val_score(rfr, prev.X_St, prev.y, 
                                        scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                rfr_scores = np.append(rfr_scores, 
                                    cross_val_score(rfr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.svr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'No':
                        for fold in folds_list:
                            if self.continue_flag:
                                svr_scores = np.append(svr_scores, 
                                    cross_val_score(svr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                svr_scores = np.append(svr_scores, 
                                    cross_val_score(svr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.sgdr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'No':
                        for fold in folds_list:
                            if self.continue_flag:
                                sgdr_scores = np.append(sgdr_scores, 
                                    cross_val_score(sgdr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                sgdr_scores = np.append(sgdr_scores, 
                                    cross_val_score(sgdr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.knr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'No':
                        for fold in folds_list:
                            if self.continue_flag:
                                knr_scores = np.append(knr_scores, 
                                    cross_val_score(knr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                knr_scores = np.append(knr_scores, 
                                    cross_val_score(knr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.gpr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'No':
                        for fold in folds_list:
                            if self.continue_flag:
                                gpr_scores = np.append(gpr_scores, 
                                    cross_val_score(gpr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                gpr_scores = np.append(gpr_scores, 
                                    cross_val_score(gpr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.dtr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                dtr_scores = np.append(dtr_scores, 
                                    cross_val_score(dtr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                dtr_scores = np.append(dtr_scores, 
                                    cross_val_score(dtr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.mlpr_include_comp.get() and self.continue_flag:
                    from sklearn.neural_network import MLPRegressor
                    mlpr = MLPRegressor(
                        hidden_layer_sizes=tuple([int(self.mlpr_hidden_layer_sizes.get())]),
                    ...
                    if self.x_st_var.get() == 'No':
                        for fold in folds_list:
                            if self.continue_flag:
                                mlpr_scores = np.append(mlpr_scores, 
                                    cross_val_score(mlpr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                mlpr_scores = np.append(mlpr_scores, 
                                    cross_val_score(mlpr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.xgbr_include_comp.get() and self.continue_flag:
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                xgbr_scores = np.append(xgbr_scores, 
                                    cross_val_score(xgbr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                xgbr_scores = np.append(xgbr_scores, 
                                    cross_val_score(xgbr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                ...
                if self.cbr_include_comp.get() and self.continue_flag:
                    ...
                    if self.cbr_use_gpu.get() == False:
                        cbr = CatBoostRegressor(
                            ...
                            reg_lambda=float(self.cbr_reg_lambda.get()),
                    ...
                    if self.x_st_var.get() == 'Yes':
                        for fold in folds_list:
                            if self.continue_flag:
                                cbr_scores = np.append(cbr_scores, 
                                    cross_val_score(cbr, prev.X_St, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
                    else:
                        for fold in folds_list:
                            if self.continue_flag:
                                cbr_scores = np.append(cbr_scores, 
                                    cross_val_score(cbr, prev.X, prev.y, scoring='r2', cv=fold))
                                self.pb1.step(1)
        ...
        ttk.Button(self.frame, text='Load data ', 
            command=lambda: load_data(self, self.prediction, pr_data_entry, 'rgr prediction')).place(x=490, y=290)
        ...
        ttk.Button(self.frame, text='View/Change', command=lambda: 
            Data_Preview(self, self.prediction, 
                'rgr prediction', parent)).place(x=230, y=345)
        ...
        def make_regression(method):
            ...
            if method == 'CatBoost':
                ...
                if self.cbr_use_gpu.get() == False:
                    ...
                    cbr = CatBoostRegressor(iterations=int(self.cbr_iterations.get()), 
                        task_type="GPU", devices='0:1',
                        learning_rate=(None if self.cbr_learning_rate.get()=='None' 
                            else float(self.cbr_learning_rate.get())),
                        depth=int(self.cbr_depth.get()), 
        ...
        ttk.Button(self.frame, text='Predict values', 
            command=lambda: 
                try_make_regression(method=self.pr_method.get())).place(x=420, y=360)
        
        ttk.Button(self.frame, text='Save results', 
            command=lambda: save_results(self, self.prediction, 'rgr result')).place(x=420, y=400)
        ttk.Button(self.frame, text='Quit', 
                  command=lambda: quit_back(rgr_app.root, parent)).place(x=420, y=440)
...
class rgr_mtds_specification:
    def __init__(self, prev, parent):
        ...
        ttk.Label(self.frame, text=' Include in\ncomparison', font=myfont2).place(x=1075, y=40)
        xgb_cb1 = ttk.Checkbutton(self.frame, variable=prev.xgbr_include_comp, takefocus=False)
        xgb_cb1.place(x=1165, y=50)
        ttk.Label(self.frame, text='Learning rate', font=myfont2).place(x=1055, y=80)
        xgb_e1 = ttk.Entry(self.frame, textvariable=prev.xgbr_eta, font=myfont2, width=7)
        xgb_e1.place(x=1165,y=80)
        ttk.Label(self.frame, text='Min Child Weight', font=myfont2).place(x=1055, y=100)
        xgb_e2 = ttk.Entry(self.frame, 
            textvariable=prev.xgbr_min_child_weight, font=myfont2, width=7)
        xgb_e2.place(x=1165,y=100)
        ttk.Label(self.frame, text='Max Depth', font=myfont2).place(x=1055, y=120)
        xgb_e3 = ttk.Entry(self.frame, textvariable=prev.xgbr_max_depth, font=myfont2, width=7)
        xgb_e3.place(x=1165,y=120)
        ttk.Label(self.frame, text='Gamma', font=myfont2).place(x=1055, y=140)
        xgb_e4 = ttk.Entry(self.frame, textvariable=prev.xgbr_gamma, font=myfont2, width=7)
        xgb_e4.place(x=1165,y=140)
        ttk.Label(self.frame, text='Subsample', font=myfont2).place(x=1055, y=160)
        xgb_e5 = ttk.Entry(self.frame, textvariable=prev.xgbr_subsample, font=myfont2, width=7)
        xgb_e5.place(x=1165,y=160)
        ttk.Label(self.frame, text='Colsample bytree', font=myfont2).place(x=1055, y=180)
        xgb_e6 = ttk.Entry(self.frame, 
            textvariable=prev.xgbr_colsample_bytree, font=myfont2, width=7)
        xgb_e6.place(x=1165,y=180)
        ttk.Label(self.frame, text='Lambda', font=myfont2).place(x=1055, y=200)
        xgb_e7 = ttk.Entry(self.frame, textvariable=prev.xgbr_lambda, font=myfont2, width=7)
        xgb_e7.place(x=1165,y=200)
        ttk.Label(self.frame, text='Alpha', font=myfont2).place(x=1055, y=220)
        xgb_e8 = ttk.Entry(self.frame, textvariable=prev.xgbr_lambda, font=myfont2, width=7)
        xgb_e8.place(x=1165,y=220)
        ttk.Label(self.frame, text='n estimators', font=myfont2).place(x=1055, y=240)
        xgb_e9 = ttk.Entry(self.frame, textvariable=prev.xgbr_n_estimators, font=myfont2, width=7)
        xgb_e9.place(x=1165,y=240)
        ttk.Label(self.frame, text='Use GPU', font=myfont2).place(x=1055, y=260)
        xgb_cb2 = ttk.Checkbutton(self.frame, variable=prev.xgbr_use_gpu, takefocus=False)
        xgb_cb2.place(x=1165, y=260)

        ttk.Label(self.frame, text='CatBoost', font=myfont_b).place(x=1085, y=280)
        ttk.Label(self.frame, text=' Include in\ncomparison', font=myfont2).place(x=1075, y=300)
        cb_cb1 = ttk.Checkbutton(self.frame, variable=prev.cbr_include_comp, takefocus=False)
        cb_cb1.place(x=1165, y=310)
        ttk.Label(self.frame, text='Iterations', font=myfont2).place(x=1055, y=340)
        cb_e1 = ttk.Entry(self.frame, textvariable=prev.cbr_iterations, font=myfont2, width=7)
        cb_e1.place(x=1165,y=340)
        ttk.Label(self.frame, text='Learning rate', font=myfont2).place(x=1055, y=360)
        cb_e2 = ttk.Entry(self.frame, textvariable=prev.cbr_learning_rate, font=myfont2, width=7)
        cb_e2.place(x=1165,y=360)
        ttk.Label(self.frame, text='Depth', font=myfont2).place(x=1055, y=380)
        cb_e3 = ttk.Entry(self.frame, textvariable=prev.cbr_depth, font=myfont2, width=7)
        cb_e3.place(x=1165,y=380)
        ttk.Label(self.frame, text='Lambda', font=myfont2).place(x=1055, y=400)
        cb_e4 = ttk.Entry(self.frame, textvariable=prev.cbr_reg_lambda, font=myfont2, width=7)
        cb_e4.place(x=1165,y=400)
        ttk.Label(self.frame, text='Subsample', font=myfont2).place(x=1055, y=420)
        cb_e5 = ttk.Entry(self.frame, textvariable=prev.cbr_subsample, font=myfont2, width=7)
        cb_e5.place(x=1165,y=420)
        ttk.Label(self.frame, text='Colsample bylevel', font=myfont2).place(x=1055, y=440)
        cb_e6 = ttk.Entry(self.frame, 
            textvariable=prev.cbr_colsample_bylevel, font=myfont2, width=7)
        cb_e6.place(x=1165,y=440)
        ttk.Label(self.frame, text='Random strength', font=myfont2).place(x=1055, y=460)
        cb_e7 = ttk.Entry(self.frame, textvariable=prev.cbr_random_strength, font=myfont2, width=7)
        cb_e7.place(x=1165,y=460)
        ttk.Label(self.frame, text='Use GPU', font=myfont2).place(x=1055, y=480)
        cb_cb2 = ttk.Checkbutton(self.frame, variable=prev.cbr_use_gpu, takefocus=False)
        cb_cb2.place(x=1165, y=480)
    ...
    def restore_defaults(self, prev):
        ...
        prev.rfr_n_jobs = tk.StringVar(value='None')
        ...
        prev.cbr_reg_lambda = tk.StringVar(value='3.0')
