...
class cl_app:
    ...
    def __init__(self, parent):
        ...
        x = (parent.ws/2) - (w/2)
        y = (parent.hs/2) - (h/2) - 30
        cl_app.root = tk.Toplevel(parent)
        cl_app.root.geometry('%dx%d+%d+%d' % (w, h, x, y))
        cl_app.root.title('Monkey Classification')
        cl_app.root.lift()
        cl_app.root.tkraise()
        cl_app.root.focus_force()
        cl_app.root.resizable(False, False)

        cl_app.root.protocol("WM_DELETE_WINDOW", lambda: quit_back(cl_app.root, parent))

        parent.iconify()
        
        self.frame = ttk.Frame(cl_app.root, width=w, height=h)
        self.frame.place(x=0, y=0)
        ...
        ttk.Button(self.frame, text='Load data ', 
            command=lambda: load_data(self, self.training, e1, 'cl training')).place(x=490, y=50)
        ...
        ttk.Button(self.frame, text='View/Change', 
            command=lambda: Data_Preview(self, self.training, 
                'cl training', parent)).place(x=230, y=95)
        #methods parameters
        ...
        self.cbc_reg_lambda = tk.StringVar(value='3.0')
        ...
        def compare_methods(prev, main):
            try:
                ...
                if self.mlpc_include_comp.get() and self.continue_flag:
                    from sklearn.neural_network import MLPClassifier
                    mlpc = MLPClassifier(
                        hidden_layer_sizes=tuple([int(self.mlpc_hidden_layer_sizes.get())]),
                ...
                if self.cbc_include_comp.get() and self.continue_flag:
                    ...
                    if self.cbc_use_gpu.get() == False:
                        cbc = CatBoostClassifier(
                            ...
                            reg_lambda=float(self.cbc_reg_lambda.get()),
        ...
        ttk.Button(self.frame, text="Methods' specifications", 
                  command=lambda: cl_mtds_specification(self, parent)).place(x=400, y=95)
        ...
        ttk.Button(self.frame, text='Load data ', 
            command=lambda: load_data(self, self.prediction, 
                pr_data_entry, 'cl prediction')).place(x=490, y=290)
        ...
        ttk.Button(self.frame, text='View/Change', command=lambda: 
            Data_Preview(self, self.prediction, 
                'cl prediction', parent)).place(x=230, y=345)
        ...
        ttk.Button(self.frame, text='Predict classes', 
            command=lambda: 
                try_cl_predict_class(method=self.pr_method.get())).place(x=420, y=360)
        
        ttk.Button(self.frame, text='Save results', 
            command=lambda: save_results(self, self.prediction, 'cl result')).place(x=420, y=400)
        ttk.Button(self.frame, text='Quit', 
            command=lambda: quit_back(cl_app.root, parent)).place(x=420, y=440)
...
class cl_mtds_specification:
    def __init__(self, prev, parent):
        ...
        xgb_cb1 = ttk.Checkbutton(self.frame, variable=prev.xgbc_include_comp, takefocus=False)
        ...
        xgb_e8 = ttk.Entry(self.frame, textvariable=prev.xgbc_lambda, font=myfont2, width=7)
        ...
        cb_cb1 = ttk.Checkbutton(self.frame, variable=prev.cbc_include_comp, takefocus=False)
        ...
        ttk.Button(self.root, text='OK', 
            command=lambda: quit_back(self.root, cl_app.root)).place(relx=0.85, rely=0.92)
